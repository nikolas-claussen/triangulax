...
*Unit mid-edge normals: normalized average of the two adjacent face
normals.*
...
Important: This function computes the exact Voronoi cell area (in any
dimension), but can lead to *numerical* instabilities for very obtuse
triangles. For use cases where the cell area serves as a tool (e.g. for
discretizing the area integral), prefer get_voronoi_areas_robust.
...
See Fig 4 from M Meyer, M Desbrun, P Schröder, A H Barr: “Discrete
...
The Gaussian curvature at a vertex can be discretized using the angle
defect:

$$K_i = \frac{1}{a_i} \left(2\pi - \sum\_{f\sim i} \phi\_{i,f} \right)$$

where the sum is over all triangles *f* neighboring vertex *i*, and
*ϕ*<sub>*i*, *f*</sub> is the angle in triangle *f* at vertex *i*. We
use the (robust) Voronoi cell area for the normalization
*a*<sub>*i*</sub> (so *K*<sub>*i*</sub> is a *density*).
...
$$H_i = -\frac{\mathbf{n}\_i \cdot (\Delta\mathbf{x})\_i}{2 a_i}$$
where and *Δ* is the cotangent Laplacian.

In both cases, we use the (robust) Voronoi cell area for the
normalization *a*<sub>*i*</sub>.
