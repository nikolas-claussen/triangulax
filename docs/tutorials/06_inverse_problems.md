...
treat the mesh as an elastic material. The elastic energy is combines
resistance to in-plane shear and dilation, and bending resistance. For
...
metric tensors for each triangle *f*. The total energy sums over all
faces, weighted by area *A*<sub>*f*</sub>. *K* and *G* are the bulk and
shear moduli. The overall scale is set by the shell thickness *δ*.

Bending resistance can be modeled using the dihedral angles
...
the preceding one. The goal is find *λ*<sub>*f*</sub> so that the
elastic energy equilibrium at the final time matches the target shape.
...
Let us start with a toy example where this problem does not occur: a
surface defined by an implicit equation *f*(**v**) = 0, for example an
ellipsoid.
