...
__all__ = ['get_circumcenter', 'get_oriented_triangle_area', 'get_triangle_area_from_sides', 'get_triangle_area',
           ...
           'get_triangle_normal', 'quaternion_to_rot_mat', 'get_barycentric_coordinates', 'rotate_around_axis']
...
def get_triangle_area_from_sides(u: Float[jax.Array, " dim"],
                                 v: Float[jax.Array, " dim"],
                                 ) -> Float[jax.Array, ""]:
    ...
    ----------
    u, v : Float[Array, "dim"]
        Triangle edge vectors (any dimension)
    ...
    Float[Array, ""]
        Triangle area.
    """
    return jnp.sqrt((u**2).sum()*(v**2).sum() - ((u*v).sum())**2) / 2
...
def get_triangle_area(a: Float[jax.Array, " dim"],
    ...
    ----------
    a, b, c : Float[Array, "dim"]
        Triangle vertex positions (any dimension)
    ...
    """
    u, v = b - a, c - a
    return get_triangle_area_from_sides(u, v)
...
def get_angle_between_vectors(a: Float[jax.Array, " dim"], b: Float[jax.Array, " dim"]
    ...
    ----------
    a, b : Float[Array, "dim"]
        Input vectors. Dimenension 2 or 3.
    ...
    """
    return jnp.atan2(2*get_triangle_area_from_sides(a, b), jnp.dot(a, b))
