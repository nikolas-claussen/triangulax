# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/00_triangulation_datastructure.ipynb.

# %% auto 0
__all__ = ['get_circumcenter', 'get_polygon_area', 'get_voronoi_corner_area', 'get_angle_between_vectors', 'get_rot_mat',
           'TriMesh', 'generate_ginibre_points', 'generate_poisson_points', 'generate_triangular_lattice',
           'get_adjacent_vertex_indices', 'label_plot', 'get_half_edge_arrays', 'HeMesh',
           'connect_boundary_to_infinity', 'HeAttribs', 'VertexAttribs', 'GeomMesh', 'set_voronoi_face_positions',
           'Mesh', 'cellplot', 'tree_stack', 'tree_unstack', 'flip_edge', 'get_signed_dual_he_length', 'flip_all',
           'sum_he_to_vertex_incoming', 'sum_he_to_vertex_opposite', 'get_cell_areas']

# %% ../nbs/00_triangulation_datastructure.ipynb 1
import numpy as np
...
# %% ../nbs/00_triangulation_datastructure.ipynb 2
import jax
import jax.numpy as jnp

# %% ../nbs/00_triangulation_datastructure.ipynb 4
...
# %% ../nbs/00_triangulation_datastructure.ipynb 7
...
def get_polygon_area(pts: Float[jax.Array, "n_vertices 2"]) -> Float[jax.Array, "1"]:
...
def get_voronoi_corner_area(a: Float[jax.Array, "2"],
                            b: Float[jax.Array, "2"],
                            c: Float[jax.Array, "2"], epsilon: float=1e-6) -> Float[jax.Array, "1"]:
    ...
    return jnp.where(jnp.abs(a_triangle) > epsilon, a_corner, jnp.zeros_like(a_corner))
    
def get_angle_between_vectors(a: Float[jax.Array, "dim"],
                              b: Float[jax.Array, "dim"]) -> Float[jax.Array, "1"]:
...
# %% ../nbs/00_triangulation_datastructure.ipynb 13
...
class TriMesh:
    ...
    **Methods**
    ...
    vertices : Float[jax.Array, "n_vertices dim"]
    faces : Int[jax.Array, "n_faces 3"]
    face_positions : type[Float[jax.Array, "n_faces dim"], None] = None

    def set_voronoi(self):
        """Set face positions to the Voronoi dual of the faces is computed."""
        self.face_positions = jax.vmap(get_circumcenter)(*[self.vertices[fcs] for fcs in self.faces.T])
        
    @staticmethod  
    def read_obj(filename: str, read_face_positions: bool = False, dim: int = 2) -> TriMesh:
        """
    ...
    def write_obj(self, filename: str, save_face_positions: bool=True) -> None:
        """
...
# %% ../nbs/00_triangulation_datastructure.ipynb 18
...
def generate_poisson_points(n_vertices: int, limit_x: float = 1, limit_y: float = 1) -> Float[jax.Array, "n_vertices 2"]:
...
# %% ../nbs/00_triangulation_datastructure.ipynb 21
...
def get_adjacent_vertex_indices(faces: Int[jax.Array, "n_faces 3"],
    ...
    boundary_vertices = jax.tree.flatten(igl.boundary_loop_all(faces))[0]
...
# %% ../nbs/00_triangulation_datastructure.ipynb 24
def label_plot(vertices: Float[jax.Array, "n_vertices 2"],
               faces: Int[jax.Array, "n_faces 3"],
               hemesh: type[None|HeMesh]=None,
               vertex_labels: bool = True, face_labels: bool=True,
               ax: type[None | mpl.axes.Axes]=None,
               fontsize: type[int|None] = None) -> None:
    ...
    if hemesh is not None:
        ...
        for ie in np.where(hemesh.is_unique)[0]:
            ax.text(*edge_center[ie], f"{ie}/{hemesh.twin[ie]}", color="tab:orange", ha="center", fontsize=fontsize)
            
    return None

# %% ../nbs/00_triangulation_datastructure.ipynb 26
def get_half_edge_arrays(n_vertices: int, faces: Int[jax.Array, "n_faces 3"]) -> List[Int[jax.Array, "?"]]:
    """
...
# %% ../nbs/00_triangulation_datastructure.ipynb 30
...
class HeMesh:
    ...
    **Static methods**
    ...
    heface : Int[jax.Array, "n_hes"]
    face_incident : Int[jax.Array, "n_faces"]
    inf_vertices : Tuple[int] = dataclasses.field(default=(), metadata=dict(static=True))
    ...
    def iterate_around_vertex(self: HeMesh, v: int) -> Int[jax.Array, "n_neighbors"]:
    ...
    def __eq__(self, other) -> bool:
        ...
        if not self.inf_vertices == other.inf_vertices:
            return False
        return jax.tree_util.tree_all(jax.tree.map(jnp.allclose, self, other)) # compares all array values

# %% ../nbs/00_triangulation_datastructure.ipynb 40
def connect_boundary_to_infinity(vertices: Float[jax.Array, "n_vertices 2"], faces: Int[jax.Array, "n_faces 2"]
                                ) -> Tuple[Float[jax.Array, "n_vertices+1 2"], Int[jax.Array, "n_faces_new 2"]]:
    """
    ...
    assert len(boundary_loops) == 1, "Mesh must have exactly 1 boundary"
    loop = boundary_loops[0]
    inf_index = vertices.shape[0]
    inf_faces = np.stack([[inf_index, v2, v1] for v1, v2 in zip(loop, np.roll(loop, -1))])
    
    return jnp.vstack([vertices, jnp.inf*jnp.ones_like(vertices[:1])]), jnp.vstack([faces, inf_faces])

# %% ../nbs/00_triangulation_datastructure.ipynb 50
...
# %% ../nbs/00_triangulation_datastructure.ipynb 52
@jax.tree_util.register_dataclass
@dataclasses.dataclass(repr=False, eq=False, frozen=True,)
class GeomMesh:
    """
    ...
    **Static methods**
    ...
    vertices : Float[jax.Array, "n_vertices dim"]
    face_positions : Float[jax.Array, "n_faces 3"] = dataclasses.field(default_factory=lambda : jnp.array([]))
    ...
    def load(file: str) -> GeomMesh:
        ...
        return GeomHeMesh(n_vertices=npzfile['n_vertices'].item(),
...
# %% ../nbs/00_triangulation_datastructure.ipynb 53
def set_voronoi_face_positions(geommesh: GeomMesh, hemesh: HeMesh) -> GeomMesh:
    ...
    face_positions = jax.vmap(get_circumcenter)(*geommesh.vertices[hemesh.faces].transpose((1,0,2)))
    return dataclasses.replace(geommesh, face_positions=face_positions)

# %% ../nbs/00_triangulation_datastructure.ipynb 54
class Mesh(NamedTuple):
    """Combine geometric and connectivity info into a single object."""
    geommesh: GeomMesh
    hemesh: HeMesh

# %% ../nbs/00_triangulation_datastructure.ipynb 59
def cellplot(hemesh: HeMesh, face_positions: Float[jax.Array, "n_faces 2"],
             cell_colors: type[None|str|Float[jax.Array, "4"]|Float[jax.Array, "n_vertices 4"]]=None,
             mpl_polygon_kwargs=type[None|Dict[str, Any]], ax: type[None|mpl.axes.Axes]=None) -> List[mpl.patches.Polygon]:
    """
    ...
    for v in range(hemesh.n_vertices):
        if not bdry[v]:
            ...
            ax.add_patch(polygons[-1])
    return polygons

# %% ../nbs/00_triangulation_datastructure.ipynb 67
...
# %% ../nbs/00_triangulation_datastructure.ipynb 75
@functools.partial(jax.jit, static_argnames=['check_boundary'])
def flip_edge(hemesh: HeMesh, e: int, check_boundary: bool = False) -> HeMesh:
    """
...
# %% ../nbs/00_triangulation_datastructure.ipynb 76
...
# %% ../nbs/00_triangulation_datastructure.ipynb 88
@jax.jit
...
# %% ../nbs/00_triangulation_datastructure.ipynb 94
# gather-scatter operations - sum from half-edges to vertices etc. Many other operations can be done similarly

def sum_he_to_vertex_incoming(hemesh: HeMesh, he_field: jnp.ndarray):
    """
...
def sum_he_to_vertex_opposite(hemesh: HeMesh, he_field: jnp.ndarray):
    """
...
# %% ../nbs/00_triangulation_datastructure.ipynb 97
def get_cell_areas(geommesh: GeomMesh, hemesh: HeMesh) -> Float[jax.Array, "n_vertices"]:
    ...
    for v in range(hemesh.n_vertices):
        if bdry[v]:
            ...
            areas[v] = get_polygon_area(polygon)
